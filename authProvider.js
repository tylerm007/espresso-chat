...
function AppAuthenticationProviderCreate() {
	...
	function authenticate(payload) {
		...
		var result = scriptRunner.query('SELECT * FROM users WHERE uq_username = "' + payload.uq_username + '" AND password = concat(md5("' + payload.password + '"), "=chat")');
		...
		if (result) {
			try {
				...
				return {
					...
					roleNames: ['user'],
					userData: {
						username: json[0].uq_username
					},
					userInfo: {
						id: json[0].id,
						uq_username: json[0].uq_username,
						email: json[0].email
					}
				};
			} catch (e) {
